  ...
  for(tinyxml2::XMLElement* pElem=pRoot->FirstChildElement(); pElem; pElem=pElem->NextSiblingElement())
  {
    	std::string elementName = pElem->Value();
			...
			if(elementName=="window")
			{
				...
				// int resol;
				// pElem->QueryIntAttribute("resol",&resol);
				// worker.setResolution(resol);
			}
    }
...
void QtWorkerObj::Run(int argc, char** argv)
{
	...
	window->setResolution(90);		// HARD-CODED resolution
...
void QtWorkerObj::setResolution(int resol)
{
	if(!QMetaObject::invokeMethod(window, "setResolution", Qt::QueuedConnection, Q_ARG(int, resol)))
		printf(" Failed to invoke method\n");
		;
}
