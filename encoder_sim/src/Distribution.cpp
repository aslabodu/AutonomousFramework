...
double Normal::GetRV()
{
	if (_isSavedRV) {
		...
		_savedRV = (u1*sin(u2))*sqrt(_variance) + _mean;
		return((u1*cos(u2))*sqrt(_variance) + _mean);
	}
}
...
double Beta::GetRV()
{
	double rv1 = (((double)rand_r(&s1)) / (((double)RAND_MAX) + 1.0));
	double rv2 = (((double)rand_r(&s2)) / (((double)RAND_MAX) + 1.0));
	double y1,y2;

	int iter = 0;

	do{
		y1 = pow(rv1, 1.0/_alpha);
		y2 = pow(rv2, 1.0/_beta);
	}while((y1+y2) >= 1 && iter++ <= 1000);
